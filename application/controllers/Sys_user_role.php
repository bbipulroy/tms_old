...
class Sys_user_role extends Root_Controller
{
    ...
    {
        $user=User_helper::get_user();
        $this->db->from($this->config->item('system_db_tms').'.'.$this->config->item('table_system_user_group'));
        ...
        $this->db->from($this->config->item('system_db_tms').'.'.$this->config->item('table_system_user_group_role'));
    ...
    private function system_save()
    {
        ...
        $this->db->update($this->config->item('system_db_tms').'.'.$this->config->item('table_system_user_group_role'));
        if(is_array($tasks))
        {
            foreach($tasks as $task_id=>$task)
            {
                ...
                Query_helper::add($this->config->item('system_db_tms').'.'.$this->config->item('table_system_user_group_role'),$data);
            }
        }
